...
class CoTSingleFrameDataset(DistributedIterableDataset):
    """
    策略1: CoT单帧预测训练数据集
    ...
    2. 帧索引提示（不计算loss）
    3. CoT推理内容（计算loss）
    4. 单个输入图像（EarthFormer第k帧预测，不计算loss）
    5. 单个目标图像（第k帧真实GT，计算loss）
    
    对于12帧预测任务，每帧都是独立的训练样本：
    - 第1帧: EarthFormer预测第1帧 → 真实GT第1帧
    - 第2帧: EarthFormer预测第2帧 → 真实GT第2帧
    - ...
    - 第12帧: EarthFormer预测第12帧 → 真实GT第12帧
    """
                    ...
                    for row_idx, row in df.iterrows():
                        try:
                            ...
                            for frame_idx in range(12):
                                ...
                                # 2. 添加帧索引提示（不计算loss）
                                frame_prompt = f"This is frame {frame_idx + 1} of 12."
                                try:
                                ...
                                # 4. 添加输入图像（EarthFormer第k帧预测，条件图像，不计算loss）
                                try:
                                    input_image = input_images[frame_idx]  # 使用对应帧的EarthFormer预测
                                ...
                                # 5. 添加目标图像（第k帧真实GT，计算loss）
                                try:
                                    target_image = target_images[frame_idx]  # 使用对应帧的真实GT
