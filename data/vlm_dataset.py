...
def _remap_path(path: str) -> str:
    """Replace dataset root prefix if running on local copy."""
    return path.replace(OLD_PREFIX, NEW_PREFIX, 1) if path.startswith(OLD_PREFIX) else path
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for row_idx, (data, image_dir) in enumerate(data_paths_per_worker_, start=row_start_id):
                # print('img_dir old :', image_dir)
                # image_dir = _remap_path(image_dir)
                # print('img_dir new :', image_dir)
                num_tokens = 0
                image_tensor_list = []
                text_ids_list = []
                sequence_plan = []
                    ...
                    if 'image' in data_item:
                        if type(data_item['image']) == list:
                            raw_images = [
                                ...
                                pil_img2rgb(Image.open(_remap_path(image)))
                                for image in data_item['image']
                            ]
                        else:
                            raw_images = [
                                # pil_img2rgb(Image.open(os.path.join(image_dir, data_item['image'])))
                                pil_img2rgb(Image.open(_remap_path(data_item['image'])))
                            ]
                    elif 'video' in data_item:
                        if type(data_item['video']) == list:
                            # 处理多个视频的情况
                            raw_images = []
                            for video in data_item['video']:
                                # path = os.path.join(image_dir, video)
                                # print('img dir', image_dir)
                                # print('video', video)
                                # path1 = _remap_path(video)
                                # print(path1)
                                # video_frames = self.frame_sampler(os.path.join(image_dir, video))
                                video_frames = self.frame_sampler(_remap_path(video))
                                # print('video_framse:', video_frames)
                                raw_images.extend(video_frames)
                        else:
                            # 处理单个视频的情况
                            # raw_images = self.frame_sampler(os.path.join(image_dir, data_item['video']))
                            raw_images = self.frame_sampler(_remap_path(data_item['video']))
                        special_tokens = '<image>' * len(raw_images)
                        found = False
                        for item in data_item['conversations']:
                            if item.get('from') == 'human' and '<video>' in item['value']:
