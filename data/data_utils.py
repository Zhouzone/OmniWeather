...
def pil_img2gray(image):
    """Convert PIL image to grayscale for radar data processing"""
    if image.mode == "RGBA" or image.info.get("transparency", None) is not None:
        image = image.convert("RGBA")
...
def pil_img2radar(image):
    """Convert PIL image to format suitable for radar VAE (1-channel)"""
    # Convert to grayscale first
    gray_image = pil_img2gray(image)
    
    # Convert back to RGB format for compatibility with existing pipeline
    # but keep it as grayscale data (all 3 channels have same values)
