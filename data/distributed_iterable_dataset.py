...
class DistributedIterableDataset(torch.utils.data.IterableDataset):
    ...
    def set_epoch(self, seed=42):
        ...
        # 修改数据分片逻辑，确保每个rank至少能分到一个文件
        if len(data_paths) < self.world_size:
            # 如果文件数量少于rank数量，使用轮询分配
            # 每个rank分配到的文件数量 = ceil(文件总数 / rank总数)
            files_per_rank = (len(data_paths) + self.world_size - 1) // self.world_size
            start_idx = self.local_rank * files_per_rank
            end_idx = min(start_idx + files_per_rank, len(data_paths))
            
            if start_idx < len(data_paths):
                self.data_paths_per_rank = data_paths[start_idx:end_idx]
            else:
                # 如果这个rank没有分配到文件，从第一个文件开始分配
                self.data_paths_per_rank = [data_paths[self.local_rank % len(data_paths)]]
