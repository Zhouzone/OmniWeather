...
def load_forcastor_radar_data(parquet_file_path, max_samples=None):
    """加载雷达预测数据集 - 适配forcastor数据集格式"""
    print(f"Loading forcastor radar data from: {parquet_file_path}")
    
    # 使用与数据集相同的提示文本
    prompt_text = "Given 10 frames VIL (Vertically Integrated Liquid) data, predict the next 12 frames of VIL (Vertically Integrated Liquid) data over the next 1 hour. Notice that the model input are initial predictions from the EarthFormer model for the next 12 frames of 1-hour VIL data."
    ...
    try:
        ...
        for row_idx, row in tqdm(df.iterrows(), total=len(df), desc="Loading samples"):
            try:
                ...
                sample = {
                    'input_images': input_images,  # 12张输入图像
...
def main(data_dir, output_folder, seed=42, thinking=False, offload_folder=None, 
    ...
    config_info = {
        ...
        'description': 'Radar forecast generation using BAGEL model with 12 input images and text prompt to generate 12 future images',
        'dataset_type': 'forcastor_radar'
    }
    ...
    # 创建结果记录
    results = []
    json_output_path = os.path.join(output_folder, 'forcastor_radar_inference_results.json')
    
    # 处理每个parquet文件
    for parquet_file in tqdm(parquet_files, desc="Processing parquet files"):
        parquet_path = os.path.join(data_dir, parquet_file)
        samples = load_forcastor_radar_data(parquet_path, max_samples)
        ...
        for sample_idx, sample in enumerate(tqdm(samples, desc=f"Processing {parquet_file}")):
            ...
            # 使用文本提示和所有12张输入图像作为条件进行生成
            print(f"Processing sample {sample_idx} with {len(input_images)} input images and text prompt")
            ...
            try:
                ...
                print(f"Error generating images for sample {sample_idx}: {gen_error}")
                continue
    ...
    print(f"\n{'='*60}")
    print("FORCASTOR RADAR GENERATION SUMMARY REPORT")
    print(f"{'='*60}")
    ...
    print(f"Dataset type: forcastor_radar")
    print(f"Summary report saved to: {summary_path}")
    print(f"{'='*60}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="雷达预测数据集推理脚本 - Forcastor版本")
    ...
    parser.add_argument('--output_folder', type=str, required=False,
                       default='/mnt/shared-storage-user/zhouzhiwang/omni-weather/results/forcastor_radar_gen_test',
                       help='输出文件夹路径')
