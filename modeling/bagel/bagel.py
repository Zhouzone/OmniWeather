...
class Bagel(PreTrainedModel):
    ...
    def _get_text_embeddings(self, text_ids):
        """获取文本嵌入，兼容LoRA包装后的模型结构"""
        if hasattr(self.language_model, 'get_input_embeddings'):
            return self.language_model.get_input_embeddings()(text_ids)
        elif hasattr(self.language_model, 'base_model'):
            # 处理PeftModel包装的情况
            return self.language_model.base_model.model.embed_tokens(text_ids)
        else:
            return self.language_model.model.embed_tokens(text_ids)
    ...
    def forward_cache_update_vae(
        ...
        padded_latent = vae_model.encode(padded_images.to(dtype=packed_sequence.dtype, device=packed_sequence.device))
