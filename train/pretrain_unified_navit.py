...
# ====== Gloo-only gather，避免在 NCCL 上收集 Python 对象 ======
# def _ensure_gloo_group():
#     # 缓存一份 Gloo 通信组，避免每次重建
#     if not hasattr(_ensure_gloo_group, "pg"):
#         _ensure_gloo_group.pg = dist.new_group(backend="gloo")
#     return _ensure_gloo_group.pg

# def gloo_gather_object(obj, dst=0):
#     """
#     用 Gloo 后端收集小 Python 对象。
#     仅 dst(rank0) 返回 list，其它 rank 返回 None。
#     """
#     pg = _ensure_gloo_group() 
#     rank = dist.get_rank()
#     world = dist.get_world_size()
#     gather_list = [None] * world if rank == dst else None
#     # 在通信前把潜在 CUDA 异常抖出来，避免拖到通信时报错
#     torch.cuda.synchronize()
#     dist.barrier()
#     dist.gather_object(obj, gather_list, dst=dst, group=pg)
#     dist.barrier()
...
class ModelArguments:
    model_path: str = field(
        default="pretrained/ByteDance-Seed/BAGEL-7B-MoT",
...
def main():
    ...
    logger.info(f"Start to build llm model")
    # Setup model:
    ...
    logger.info(f"Start to build vit model")
    ...
    logger.info(f"Start to build model")
    config = BagelConfig(
    ...
    logger.info(f"Start to build tokenizer")
    ...
    if resume_model_only:
        print('resume model only')
        train_step = 0
        data_status = None
    else:
        print('resume optimizer, scheduler, and train_steps')
